...
describe('StoreProvider', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({ json: () => Promise.resolve(CATALOG) }),
    );
  });

  it('acumula unidades y subtotal al añadir al carrito', async () => {
  ...
  it('muestra un toast con CTA al añadir al carrito', async () => {
    const { result } = renderHook(() => useStore(), { wrapper });
    await waitFor(() => expect(result.current.productMap.size).toBe(CATALOG.length));
    act(() => result.current.addToCart(1, 1));
